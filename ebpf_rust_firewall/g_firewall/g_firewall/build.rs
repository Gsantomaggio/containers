...
/// This crate has a runtime dependency on artifacts produced by the `g_firewall-ebpf` crate.
...
/// crate's undeclared dependency on artifacts from `g_firewall-ebpf` would cause build (and
...
/// performance; an environment variable is used to replace true builds of `g_firewall-ebpf`
/// with stubs to preserve the property that code generation and linking (in
/// `g_firewall-ebpf`) do not occur on metadata-only actions such as `cargo check` or `cargo
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "g_firewall-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `g_firewall-ebpf`, so cargo will automatically rebuild us
        // if `g_firewall-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `g_firewall-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("g_firewall-ebpf");
