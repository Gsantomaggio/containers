...
1. (if cross-compiling) rustup target: `rustup target add arm-unknown-linux-musl`
...
```shell
AYA_BUILD_EBPF=true CC=${ARCH}-linux-musl-gcc cargo build --package g_firewall --release \
  --target=${ARCH}-unknown-linux-musl \
  --config=target.${ARCH}-unknown-linux-musl.linker=\"${ARCH}-linux-musl-gcc\"
```
The cross-compiled program `target/${ARCH}-unknown-linux-musl/release/g_firewall` can be
