...
/// This crate has a runtime dependency on artifacts produced by the `ebpf_rust_firewall-ebpf` crate.
...
/// crate's undeclared dependency on artifacts from `ebpf_rust_firewall-ebpf` would cause build (and
...
/// `ebpf_rust_firewall-ebpf`.
///
/// Note that this solution is imperfect: in particular it has to balance correctness with
/// performance; an environment variable is used to replace true builds of `ebpf_rust_firewall-ebpf`
/// with stubs to preserve the property that code generation and linking (in
/// `ebpf_rust_firewall-ebpf`) do not occur on metadata-only actions such as `cargo check` or `cargo
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "ebpf_rust_firewall-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `ebpf_rust_firewall-ebpf`, so cargo will automatically rebuild us
        // if `ebpf_rust_firewall-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `ebpf_rust_firewall-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("ebpf_rust_firewall-ebpf");
