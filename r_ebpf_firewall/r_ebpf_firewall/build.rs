...
/// This crate has a runtime dependency on artifacts produced by the `r_ebpf_firewall-ebpf` crate.
...
/// crate's undeclared dependency on artifacts from `r_ebpf_firewall-ebpf` would cause build (and
...
/// `r_ebpf_firewall-ebpf`.
///
/// Note that this solution is imperfect: in particular it has to balance correctness with
/// performance; an environment variable is used to replace true builds of `r_ebpf_firewall-ebpf`
/// with stubs to preserve the property that code generation and linking (in
/// `r_ebpf_firewall-ebpf`) do not occur on metadata-only actions such as `cargo check` or `cargo
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "r_ebpf_firewall-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `r_ebpf_firewall-ebpf`, so cargo will automatically rebuild us
        // if `r_ebpf_firewall-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `r_ebpf_firewall-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("r_ebpf_firewall-ebpf");
